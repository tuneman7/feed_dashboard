...
# This script removes the EC2 instance from the RDS security group to revoke database connections
...
# Configuration - Update these values with your specific resources
RDS_INSTANCE="dst-dashboard-database-fast1"
EC2_INSTANCE_ID="i-0a8e38558026a9e78"
EC2_SECURITY_GROUP_ID="sg-0a8d0b864b0075c1f"
EC2_PUBLIC_IP="54.196.102.62"

if [ -z "$RDS_INSTANCE" ]; then
    echo "❌ No RDS instance identifier provided"
    echo "Please update the RDS_INSTANCE variable in this script"
    exit 1
fi
...
# For each RDS security group, remove the EC2 security group and IP access
for RDS_SG_ID in $RDS_SECURITY_GROUPS; do
    ...
    if [ -n "$EXISTING_RULE" ]; then
        ...
        echo "ℹ️  EC2 security group rule not found in RDS security group"
    fi
    ...
    if [ -n "$EXISTING_IP_RULE" ]; then
        ...
        echo "✅ Removed EC2 public IP rule ($EC2_PUBLIC_IP/32)"
    else
        echo "ℹ️  EC2 public IP rule not found in RDS security group"
    fi
...
echo "📋 Summary:"
echo "- Removed security group: $EC2_SECURITY_GROUP_ID from RDS access"
echo "- Removed public IP: $EC2_PUBLIC_IP/32 from RDS access" 
echo "- RDS endpoint: $RDS_ENDPOINT"
echo
echo "🔒 The specified EC2 instance can no longer connect to RDS!"
echo "💡 If you need to restore access, run the corresponding 'fix' script"